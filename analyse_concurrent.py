...
def load_config_or_store(store: Store, input_file: str, output_path: str):
    ...
    if '' in store.Solution:
        ...
        dataroot = f"{g.convergemod}_{g.nff}_{int(g.iff*g.model_scale*10000)}_{int(g.fff*g.model_scale*10000)}"
    else:
        # Running from in-memory store
        config = store.Configuration
        dataroot = list(config.keys())[0]
        g = config[dataroot]
        T, M, S, D, R, Neltoverlambda, nRD, _ = cfg_tuple
    return T, M, S, D, R, Neltoverlambda, nRD, g, dataroot
...
def analyse_concurrent(store: Store, input_file: str, output_path: str, task_name: str) -> Store:
    ...
    T, M, S, D, R, Neltoverlambda, nRD, g, dataroot = load_config_or_store(store, input_file, output_path)
