...
for ii = 1:size(doma,2)
   ...
   resp{2,ii}(1) = r1(ii);
   resp{3,ii}(1) = r2(ii);
   doma{1,ii}(:,1) = field.range.pid(ii,:);
   doma{2,ii}(:,1) = field.range.prr(ii,:);
   doma{3,ii}(:,1) = field.range.prl(ii,:);
end
...
for ii = (g.prop.nfi+1):4:(g.prop.nfr-3);
  display(ii+4)
  ...
  for jj = 0:3
    %resp{ii+jj) = out(jj+1).rr;
    %doma(ii+jj) = out(jj+1).dd;
    ij = ii+jj;
    rr1 = out(jj+1).rr;
    dd1 = out(jj+1).dd;
