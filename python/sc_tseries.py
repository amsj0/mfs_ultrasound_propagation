...
numbr_frequencies = 50
initi_frequencies = 2
final_frequencies = 100
parti_frequencies = 50
converge = 3
...
x = np.arange(0.0,10e-6,1/(spec_size*2e4))[:,None]
x_size = np.size(x,axis=0)
x_spec = np.arange(0.0,1/2e4,1/(spec_size*2e4))[:,None]
x_spec_full = np.arange(0.0,factor/2e4,1/(spec_size*2e4))[:,None]
...
for ii in range(0+1*int(1*x_size*4/8),1+1*int(1*x_size*4/8),1):
    ...
    for jj in range(0,int(doma_dims[0]/1),23):
        ...
        doma_data1 = np.array(doma_dataset[doma_list_keys[index]]['value']).view(np.complex)
        ...
        for ll in range(int(factor*int(spec_size/16))):
            this_series = mat_tseries[ll*16,:,0].real/vlim
            #this_series = 20*np.log10(abs(mat_tseries[ll*16,:,0]))
            this_series.shape = this_grid.shape
            h0.set_array(this_series.ravel())
            
            this_series = mat_tseries[ll*16,:,1].real/vlim
            #this_series = 20*np.log10(abs(mat_tseries[ll*16,:,1]))
            this_series.shape = this_grid.shape            
            h1.set_array(this_series.ravel())
            
            this_series = mat_tseries[ll*16,:,2].real/vlim
            #this_series = 20*np.log10(abs(mat_tseries[ll*16,:,2]))
            this_series.shape = this_grid.shape            
            h2.set_array(this_series.ravel())
            
            this_series = mat_tseries[ll*16,:,3].real/vlim
            #this_series = 20*np.log10(abs(mat_tseries[ll*16,:,2]))
            this_series.shape = this_grid.shape            
            h3.set_array(this_series.ravel())
            
            
            fg.suptitle('time-step ' + str(x_spec_full[ll*16]) + ' | height-step ' + str(jj))
            #domat.shape
